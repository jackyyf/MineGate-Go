...
if __name__ == '__main__':
	...
	parser.add_argument('packages', nargs='*', help='List of packages.', default=[])
	...
	if args.action in ['add', 'delete']:
		...
		if args.action == 'add':
			...
			package_set = set(packages)
			package_set = package_set.difference(args.packages if isinstance(args.packages, (list, tuple)) else [args.packages])
			with open('.plugins', 'w') as f:
				f.write('\n'.join(package_set))
		sys.exit(0)
		...
		if packages:
			with open('minegate/plugin.go', 'w') as f:
				code_data = code_template.format(
					import_packages='\n'.join(
						map(lambda s : '\t_ "%s"' % s.replace('"', '\\"'), packages)
					)
				)
				f.write(code_data)
		else:
